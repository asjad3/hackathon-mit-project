...
from app.models.context import ContextState, WeatherContext, EventInfo, MerchantDemand
...
async def get_full_context(
    ...
    return await assemble_context(lat, lng)


@router.get("/weather", response_model=WeatherContext)
async def get_weather_context(city: str = Query(default=None)):
...
async def get_events(
    ...
    lat = lat if lat is not None else settings.default_lat
    lng = lng if lng is not None else settings.default_lng
    return await get_nearby_events(lat, lng, radius_km)
